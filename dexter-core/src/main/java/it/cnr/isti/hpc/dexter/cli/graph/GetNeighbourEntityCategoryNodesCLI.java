...
/**
 * Given an entity (category) ID or a entity (category) label as input, and a
 * direction (in/out) returns the categories linking to the given entity (in) or
 * the categories linked from the given entity (out).
 * 
 * @author Diego Ceccarelli, diego.ceccarelli@isti.cnr.it created on 21/nov/2011
 */
public class GetNeighbourEntityCategoryNodesCLI extends
	...
	private static final Logger logger = LoggerFactory
			.getLogger(GetNeighbourEntityCategoryNodesCLI.class);

	private static String[] params = new String[] { "input", "direction" };

	private static final String USAGE = "java -cp $jar "
			+ GetNeighbourEntityCategoryNodesCLI.class
			+ " -input id/label -direction in/out";

	public static void main(String[] args) {
		GetNeighbourEntityCategoryNodesCLI cli = new GetNeighbourEntityCategoryNodesCLI(
				args);

		String direction = cli.getParam("direction");
		NodeStar sn = null;
		if (direction.equals("in")) {
			sn = EntityCategoryNodeFactory
	...
	public GetNeighbourEntityCategoryNodesCLI(String[] args) {
