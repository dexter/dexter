...
/**
 * The Document interface defines as a set of {@link Field fields}. Each field
 * has a name and a textual value. Thus each document should typically contain
 * one or more fields which uniquely identify it. <br/>
 * A document can contain only one field with a fixed name (duplicate fields are
 * not supported).
 * 
...
public abstract class Document implements Serializable {
	...
	// public static class DocumentDeserializer implements
	// JsonDeserializer<Document> {
	//
	// private static Gson gson = new Gson();
	//
	// public Document deserialize(JsonElement json, Type arg1,
	// JsonDeserializationContext arg2) throws JsonParseException {
	// return gson.fromJson(json, MultifieldDocument.class);
	// }
	//
	// }
